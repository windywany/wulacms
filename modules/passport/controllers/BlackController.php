...
class BlackController extends Controller {
	...
	public function add($id = 0) {
		...
		if($id>0){
			$rs = dbselect('*')->from('{member_nickname_black}')->where(array('id'=>$id))->get();
		}
		$data = $rs;
	...
	public function save() {
		...
		if ($rs) {
			...
			if (empty ( $id )) {
				...
				$rst = dbinsert ( $rs )->into ( '{member_nickname_black}' )->exec ();
			} else {
				// 修改
				$rst = dbupdate ( '{member_nickname_black}' )->set ( $rs )->where ( array ('id' => $id ) )->exec ();
			}
			if ($rst) {
