	...
	public function reply_post($id, $content) {
		...
		if ($id && $content) {
			...
			if ($comment) {
				...
				if ($rst) {
					dbupdate('{comments}')->set(array('status' => 1, 'reply_count' => imv('reply_count+1')))->where(array('id' => $id))->exec();
					dbupdate('{cms_page}')->set(['comments' => imv('comments+1')])->where(['id' => $data['page_id']])->exec();
