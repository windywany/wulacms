...
class Wechat {
	...
	public function valid($return = false) {
		...
		if ($_SERVER['REQUEST_METHOD'] == "POST") {
			...
			if ($this->encrypt_type == 'aes') { //aes加密
				...
				if (!$this->appid) $this->appid = $array[2];//为了没有appid的订阅号。
			} else {
			...
			if ($return) {
				if ($this->checkSignature()) return $echoStr; else
					return false;
			} else {
				if ($this->checkSignature()) die($echoStr); else
					die('no access');
			}
		}
	...
	public function Message($msg = '', $append = false) {
		if (is_null($msg)) {
			...
			if ($append) $this->_msg = array_merge($this->_msg, $msg); else
				$this->_msg = $msg;
	...
	public function getRevFrom() {
		if (isset($this->_receive['FromUserName'])) return $this->_receive['FromUserName']; else
			return false;
	}
	...
	public function getRevTo() {
		if (isset($this->_receive['ToUserName'])) return $this->_receive['ToUserName']; else
			return false;
	}
	...
	public function getRevType() {
		if (isset($this->_receive['MsgType'])) return $this->_receive['MsgType']; else
			return false;
	}
	...
	public function getRevID() {
		if (isset($this->_receive['MsgId'])) return $this->_receive['MsgId']; else
			return false;
	}
	...
	public function getRevCtime() {
		if (isset($this->_receive['CreateTime'])) return $this->_receive['CreateTime']; else
			return false;
	}
	...
	public function getRevPic() {
		if (isset($this->_receive['PicUrl'])) return array('mediaid' => $this->_receive['MediaId'], 'picurl' => (string)$this->_receive['PicUrl'],    //防止picurl为空导致解析出错
		); else
			return false;
	}
	...
	public function getRevCardPass() {
		if (isset($this->_receive['CardId'])) return $this->_receive['CardId']; else
			return false;
	}
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename, $value, $expired) {
		@file_put_contents(TMP_PATH.$cachename,$value.'====='.(time() + $expired));
		return true;
	}
	...
	protected function getCache($cachename) {
		if(is_file(TMP_PATH.$cachename)){
			$cnt = file_get_contents(TMP_PATH.$cachename);
			if($cnt){
				$cnts = explode('=====',$cnt);
				if(isset($cnts[1]) && $cnts[1] > time()){
					return $cnts[0];
				}
			}
		}
	...
	protected function removeCache($cachename) {
		@file_put_contents(TMP_PATH.$cachename,'');
		return true;
	}
	...
	public function getCardSign($card_type = '', $card_id = '', $code = '', $location_id = '', $timestamp = 0, $noncestr = '', $appid = '') {
		...
		if (!$sign) return false;
		$signPackage = array("cardType" => $card_type, "cardId" => $card_id, "shopId" => $location_id,         //location_id就是shopId
		                     "nonceStr" => $noncestr, "timestamp" => $timestamp, "cardSign" => $sign);
	...
	static function json_encode($arr) {
		...
		foreach ($arr as $key => $value) {
			if (is_array($value)) { //Custom handling for arrays
				if ($is_list) $parts [] = self::json_encode($value); /* :RECURSION: */ else
					$parts [] = '"' . $key . '":' . self::json_encode($value); /* :RECURSION: */
			} else {
				$str = '';
				if (!$is_list) $str = '"' . $key . '":';
				//Custom handling for multiple data types
				if (!is_string($value) && is_numeric($value) && $value < 2000000000) $str .= $value; //Numbers
	...
	public function getSignature($arrdata, $method = "sha1") {
		...
		foreach ($arrdata as $key => $value) {
			if (strlen($paramstring) == 0) $paramstring .= $key . "=" . $value; else
	...
	public function getQRCode($scene_id, $type = 0, $expire = 604800) {
		...
		switch ($type) {
			case '0':
				if (!is_numeric($scene_id)) return false;
			...
			case '2':
				if (!is_string($scene_id)) return false;
	...
	public function getDatacube($type, $subtype, $begin_date, $end_date = '') {
		if (!$this->access_token && !$this->checkAuth()) return false;
		if (!isset(self::$DATACUBE_URL_ARR[ $type ]) || !isset(self::$DATACUBE_URL_ARR[ $type ][ $subtype ])) return false;
	...
	public function getUserGroup($openid) {
		...
		if ($result) {
			...
			if (!$json || !empty($json['errcode'])) {
				...
				return false;
			} else if (isset($json['groupid'])) return $json['groupid'];
		}
	...
	public function getOauthAuth($access_token, $openid) {
		...
		if ($result) {
			...
			if (!$json || !empty($json['errcode'])) {
				...
				return false;
			} else if ($json['errcode'] == 0) return true;
		}
	...
	public function createCardQrcode($card_id, $code = '', $openid = '', $expire_seconds = 0, $is_unique_code = false, $balance = '') {
		...
		if ($expire_seconds) $data['expire_seconds'] = $expire_seconds;
	...
	public function setCardTestWhiteList($openid = array(), $user = array()) {
		$data = array();
		if (count($openid) > 0) $data['openid'] = $openid;
		if (count($user) > 0) $data['username'] = $user;
		if (!$this->access_token && !$this->checkAuth()) return false;
