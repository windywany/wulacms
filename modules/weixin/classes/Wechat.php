...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 * @return boolean
	 * 
	 * modified DQ 设置缓存生效，使用weixin_wechat 分组
	 * 
	 */
	protected function setCache($cachename,$value,$expired){
		set_cfg ( $cachename, $value, 'weixin_wechat' );
		set_cfg ( $cachename.'_expire', time () + $expired, 'weixin_wechat' );
		return true;
	}
	...
	protected function getCache($cachename){
		$val = cfg ( $cachename.'@weixin_wechat', null );
		$expire = icfg ( $cachename.'_expire@weixin_wechat', 0 );
		if ($val && $expire > time ()) {
			return $val;
		}
		return false;
	...
	protected function removeCache($cachename){
		set_cfg ( $cachename, '', 'weixin_wechat' );
		set_cfg ( $cachename.'_expire', 0, 'weixin_wechat' );
		return true;
	}
