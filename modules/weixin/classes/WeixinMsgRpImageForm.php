 ...
 *
 */
class WeixinMsgRpImageForm extends AbstractForm {
	...
	public function save($data = array(), $msgId = 0){
	    ...
	    $exist = dbselect('*')->from('{weixin_msg_rp_image}')->where(array('msg_id'=>$msgId,'table'=>$data['table']))->desc('id')->get();
	    ...
	    if($exist['id']>0){
	        $return = dbupdate('{weixin_msg_rp_image}')->set($save)->where(array('id'=>$exist['id']))->exec();
	    }else{
	        $save = array_merge($save,array('create_uid'=>$data['uid'],'create_time'=>$time));
	        $return = dbinsert($save)->into('{weixin_msg_rp_image}')->exec();
	    }
	    if($return === false){
	        return false;
	    }else{
