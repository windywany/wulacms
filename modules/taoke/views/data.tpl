...
{foreach $results as $c}
    ...
    <td>
        ...
        <p>
            淘口令:<strong id="tid-{$c.cid}">{$c.token}</strong>{if !$c.token}<a id="gbtn-{$c.cid}" title="淘口令" class="btn btn-xs btn-primary"
                                                 href="{'taoke/createtoken/'|app}{$c.cid}" target="ajax"
                                                 data-confirm="你确定要生成淘口令吗?">
                <i class="fa fa-pencil-square-o"></i>生成淘口令</a>{/if}
                ...
                <div class="row">
                    <div>
                        <form target="ajax" id="apply-form" method="post" action="{'taoke/taoke/saveReason'|app}" data-widget="nuiValidate" name="ApplyForm" class="smart-form">
                            ...
                            <label class="form-inline">推荐理由：<input type="text" class="form-control" name="reason" value="{$c.reason}" style="width: 250px;"/></label>
                            <label class="form-inline">是否推荐：<input type="checkbox" name="checkbox[]" class="form-control" value="1" {if $c.flag_c} checked {/if}/></label>
                            <label class="form-inline">是否特荐：<input type="checkbox" name="checkbox[]" class="form-control" value="2" {if $c.flag_a} checked {/if}/></label>
                            <label class="form-inline"><button id="submit" class="btn btn-primary" type="submit">保存</button></label>
                        </form>
                    </div>
                </div>
