...
class DiggHooksImpl {
	...
	public static function load_page_common_data($values) {
		...
		if ($page_id) {
			...
			$diggs = dbselect($fields)->from('{cms_digg}')->where(array('page_id' => $page_id))->get(0);
			if ($diggs) {
	...
	public static function mobiapp_page_query(Query $query) {
		if ($query) {
			$query->field('DIGG.digg_total');
			for ($i = 0; $i < 10; $i++) {
				if (bcfg('digg' . $i . '_enabled@digg')) {
	...
	public static function mobiapp_view_data($data, $page, $views) {
		for ($i = 0; $i < 10; $i++) {
			$f = 'digg_' . $i;
			if (isset($page[ $f ])) {
				$data[ $f ] = $page[ $f ];
