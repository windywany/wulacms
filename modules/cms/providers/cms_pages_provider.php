 ...
 * @return CtsData
 */
function cms_pages_provider($con, $tplvars = [], $dialect = null) {
	...
	if ($model) {
		if (isset ($model_fields [ $model ])) {
			...
			$fields                  = dbselect('name,id')->setDialect($dialect)->from('{cms_model_field} AS CMF')->where(array('model' => $model, 'deleted' => 0, 'cstore' => 0))->toArray();
 ...
 * @return CtsData
 */
function cms_chunk_provider($con, $tplvars, $dialect = null) {
 ...
 * @return CtsData
 */
function cms_block_provider($con, $tplvars, $dialect = null) {
	ksort($con);
 ...
 * @return CtsData
 */
function cms_channel_provider($con, $tplvars, $dialect = null) {
	...
	if (!empty ($id)) {
		...
		$noid = get_condition_value('noid', $con);

		$where = array();
		$query = dbselect('CH.*')->from('{cms_channel} AS CH')->where($where);
		if (is_numeric($upid)) {
			$where ['upid'] = abs($upid);
		} else if ($upid) {
			$query->join('{cms_channel} AS CH2', 'CH.upid=CH2.id', QueryBuilder::INNER);
			$where['CH2.refid'] = $upid;
		}
		if ($noid) {
			$ids                    = explode(',', trim($noid));
			$where ['CH.refid !IN'] = $ids;
		}
		$where ['CH.deleted'] = 0;
		$where ['CH.hidden']  = 0;
		$query->where($where);
	...
	if (isset ($query)) {
		...
		$cnt = dbselect(imv('COUNT(CP.id)'))->from('{cms_page} AS CP')->join('{cms_channel} AS CH1', 'CH1.refid = CP.channel');
		$cnt->where(array('CH1.id IN' => imv('CH.subchannels'), 'CP.deleted' => 0, 'CP.hidden' => 0));
		if ($dialect) {
			$query->setDialect($dialect);
			$cnt->setDialect($dialect);
		}
 ...
 * @return CtsData
 */
function cms_menu_provider($con, $tplvars, $dialect = null) {
	$upid = get_condition_value('upid', $con);
