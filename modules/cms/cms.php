 ...
 * @return array
 */
function get_common_page_condition_where($con, &$mfiled = null) {
	$where = array();
	...
	if ($title) {
		...
		if ($fs) {
			...
			$fss  = get_keywords(null, $title, null, $dict, true);
			if ($fss[0]) {
				$where ['CP.search_index MATCH'] = $fss[1];
				$mfiled                          = imv('MATCH(CP.search_index) AGAINST(\'' . $fss[1] . '\')', 'match_title');
			}
		}
		if (!isset($fss) || !$fss[0]) {
			$where ['CP.title LIKE'] = '%' . $title . '%';
			$mfiled                  = imv('CP.title', 'match_title');
		}
	} else {
