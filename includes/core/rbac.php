...
class Passport implements Serializable, ArrayAccess {
	...
	public function getDefaultLanding() {
		return apply_filter ( 'get_passort_landing', BASE_URL, $this );
	}
	public function logout() {
		fire ( 'on_passport_logout', $this );
		sess_del ( '_USER_LoginInfo_' . $this->type );
	}
	public function save($user = false) {
		if ($user) {
			...
			if ($user ['group_id'] && ($this->type == 'admin' || $this->type == 'vip')) {
				$group = dbselect ( 'parents,subgroups,group_name,group_refid' )->from ( '{user_group}' )->where ( array ('group_id' => $user ['group_id'] ) )->get ( 0 );
				if ($group) {
			...
			if ($this->type == 'admin') {
				...
				$metas = dbselect ( 'meta_name,meta_value' )->from ( '{user_meta}' )->where ( array ('user_id' => $user ['user_id'] ) )->toArray ( 'meta_value', 'meta_name' );
				if ($metas) {
 ...
 *         $resource, or true.
 */
function icando($resource, $passport = null) {
	if ($passport === false) {
		return false;
	}
	if (is_string ( $passport )) {
		$passport = whoami ( $passport );
	}
	$passport = $passport == null ? Passport::getPassport () : $passport;
	$type = $passport->getType ();
	$rbac = Passport::getDriver ( $type );
	if (! $rbac instanceof IRbac) {
		return false;
	}
	return $rbac->icando ( $resource, $passport );
}
/**
 * check if I a the role.
 *
 ...
 * @return boolean
 */
function iam($roles, $passport = null) {
	if (is_string ( $passport )) {
		$passport = whoami ( $passport );
	}
	$passport = $passport == null ? Passport::getPassport () : $passport;
	...
	return $rbac->iam ( $roles, $passport );
}
/**
 ...
 * @return Passport
 */
function whoami($type = 'admin', $uid = 0) {
	$passport = Passport::getPassport ( $uid, $type );
	return $passport;
}
